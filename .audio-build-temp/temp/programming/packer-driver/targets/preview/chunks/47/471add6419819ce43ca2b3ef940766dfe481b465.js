  ...
  return {
    ...
    execute: function () {
      ...
      _export("spLevelItem", spLevelItem = (_dec = ccclass('spLevelItem'), _dec2 = property({
        ...
        adjustCellSize() {
          var screenSize = this.graphicsNode.getComponent(UITransform).contentSize; // 计算最大可容纳的格子尺寸（不超出屏幕）
        ...
        drawAllRopes() {
          ...
          var graphics = this.graphicsNode.getComponent(Graphics); // 清空
          ...
          var _loop = function _loop() {
            var ropeConfig = _this._levelConfig.ropes[i]; //异步执行

            setTimeout(() => {
              _this.drawSingleRope(ropeConfig, graphics);
            }, 0);
          };

          for (var i = 0; i < this._levelConfig.ropes.length; i++) {
            _loop();
          }
        } // 绘制单条绳子（连续线段+起点标记）
        ...
        drawRopePath(graphics, points, ropeConfig, strokeColor) {
          ...
          var startY = points[points.length - 1].y;
