    ...
    execute: function () {
      ...
      _export("default", SoundManager = (_dec = ccclass('SoundManager'), _dec(_class = (_class2 = class SoundManager extends (_crd && BaseManager === void 0 ? (_reportPossibleCrUseOfBaseManager({
        ...
        init(finish) {
          ...
          var setting = SoundManager.setting; // 默认音乐
        ...
        load(name) {
          ...
          var soundName = bundleName + "://" + name; // 判断有无缓存
        ...
        playEffect(_ref8) {
          ...
          var bundleName = bundle || BundleName;
          var soundName = bundleName + "://" + name; // 静音不允许播放
        ...
        playEffectAsync(params) {
          var _this = this;

          return _asyncToGenerator(function* () {
            return new Promise(resolve => {
              _this.playEffect(_extends({}, params, {
                onPlay: audioID => {
                  resolve(audioID);
                },
                onError: () => {
        ...
        playMusic(_ref9) {
          ...
          var bundleName = bundle || BundleName;
          var soundName = bundleName + "://" + name; // 该音乐正在播放中
        ...
        playMusicAsync(params) {
          return new Promise(resolve => {
            this.playMusic(_extends({}, params, {
              onPlay: () => {
                resolve(true);
              },
              onError: () => {
        ...
        replayMusic(onPlay) {
          ...
          if (this.playingMusic.name.indexOf('://') > 0) {
            var [_bundle, _name] = this.playingMusic.name.split('://');
            this.playMusic({
              name: _name,
              bundle: _bundle,
        ...
        setMusicMute(mute, isCache) {
          ...
          if (!mute && this.playingMusic.name) {
            if (this.playingMusic.name.indexOf('://') > 0) {
              var [_bundle2, _name2] = this.playingMusic.name.split('://');
              this.playMusic({
                name: _name2,
                bundle: _bundle2,
                volume: this.playingMusic.volume
        ...
        setMusicVolumeScale(scale, isCache) {
          if (isCache === void 0) {
            isCache = false;
          }
        ...
        setEffectVolumeScale(scale, isCache) {
          if (isCache === void 0) {
            isCache = false;
          }
