    ...
    execute: function () {
      ...
      _export("TouchZoomPan", TouchZoomPan = (_dec = ccclass('TouchZoomPan'), _dec2 = property({
        ...
        onTouchMove(event) {
          ...
          if (touches.length === 2) {
            var p1 = touches[0].getUILocation();
            var p2 = touches[1].getUILocation();
            var currentDistance = Vec2.distance(p1, p2); // 关键修复：如果在 Move 过程中发现变成了双指，但还没开始记录初始值，立刻记录
            ...
            if (this._initialDistance > 0) {
              // 计算缩放倍率
              var scaleFactor = currentDistance / this._initialDistance;
              var newScale = this._initialScale.x * scaleFactor; // 限制缩放范围
        ...
        onTouchEnd(event) {
          var touches = event.getTouches(); // 如果手指少于2根，重置缩放状态
        ...
        onMouseWheel(event) {
          ...
          var scaleDiff = (scrollY > 0 ? 1 : -1) * this.wheelSpeed;
          var newScale = this.targetNode.scale.x + scaleDiff;
        ...
        limitPosition(pos, scale) {
          ...
          var contentTrans = this.targetNode.getComponent(UITransform);
          if (!viewTrans || !contentTrans) return;
          this._viewSize = viewTrans.contentSize;
          this._contentSize = contentTrans.contentSize;
          var scaledW = this._contentSize.width * scale;
          var scaledH = this._contentSize.height * scale;
          var limitX = Math.max(scaledW, this._viewSize.width) * 1 / 4;
          if (pos.x > limitX) pos.x = limitX;
          if (pos.x < -limitX) pos.x = -limitX;
          var limitY = Math.max(scaledH, this._viewSize.height) * 1 / 4;
