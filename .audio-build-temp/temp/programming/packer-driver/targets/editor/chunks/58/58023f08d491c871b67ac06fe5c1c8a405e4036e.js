    ...
    execute: function () {
      ...
      _export("RopeRun", RopeRun = (_dec = ccclass('RopeRun'), _dec(_class = class RopeRun extends Component {
        ...
        moveRunOut(ropeConfig) {
          ...
          const secondNode = ropeConfig.ropeArrays[ropeConfig.ropeArrays.length - 2]; // 计算方向向量

          const dirX = firstNode[0] - secondNode[0];
          const dirY = firstNode[1] - secondNode[1]; // console.log(`RopeManager: 计算方向向量 - X: ${dirX}, Y: ${dirY}`);
          // 如果方向向量为0，则不需要移动
          ...
          let ropeNode = this.node; // 获取Graphics组件（用于动画）
        ...
        drawArrow(graphics, ropeConfig, color, doubleDraw = false, WorldstartPos = null) {
          ...
          const startPos = WorldstartPos; // 绘制箭头（三角形+中心线）
          ...
          const rotatedRight = this.ropeManager.rotatePoint(right, startPos.x, startPos.y, angle); // 绘制旋转后的三角形
        ...
        reDrawRope(ropeConfig, color, doubleDraw = false) {
        ...
        qiaoSuiRope() {
          ...
          for (let i = 0; i < this.runOutTool.ropeArrays.length; i++) {
            ...
            const worldPos = this.ropeManager.gridToWorldPos(ropeArray[0], ropeArray[1]); //创建一个ceil节点

            const ceilNode = (_crd && app === void 0 ? (_reportPossibleCrUseOfapp({
              error: Error()
            ...
            const randomXOffset = (Math.random() - 0.5) * 300; // 创建抛物线掉落效果
        ...
        doRunError(target) {
          let firstDoBack = true;
          let chazhenTimes = 2;
          const graphics = this.getComponent(Graphics);
          if (!graphics) return;
          const firstNode = this.runOutTool.ropeArrays[this.runOutTool.ropeArrays.length - 1]; //判断firstNode距离target的距离
          ...
          const doRunBlack = () => {
            ...
            const secondNode = tempArray.ropeArrays[tempArray.ropeArrays.length - 2]; // 计算方向向量
          ...
          const doRunRed = () => {
            ...
            const currentPathNodes = backStep.pop(); // console.log('currentPathNodes',currentPathNodes);
            //回退的index坐标
