  ...
  return {
    ...
    execute: function () {
      ...
      DailyTimer = class DailyTimer {
        ...
        constructor(time, callback, target, once = false) {
          this.startMs = void 0;
          // 起始时间毫秒数（相对于当日零点）
          this.endMs = void 0;
          // 结束时间毫秒数
          ...
          this.once = once;
          // 使用解构赋值提高可读性
          const [startSegment, endSegment = startSegment] = time.split('-'); // 开始时间

          const [startH = 0, startM = 0, startS = 0] = startSegment.split(':').map(part => Math.max(0, parseInt(part, 10) || 0));
          this.startMs = DailyTimer.parseTimeToMs(startH, startM, startS); // 结束时间

          const [endH = 0, endM = 0, endS = 0] = endSegment.split(':').map(part => Math.max(0, parseInt(part, 10) || 0));
        ...
        isInRange(now) {
          const currentMs = DailyTimer.getDayTimeMs(now); // 处理跨天时间段（如 23:00-01:00）
      ...
      IntervalTimer = class IntervalTimer {
        constructor(interval, callback, target, once = false) {
          this.elapsed = 0;
        ...
        update(dt) {
          this.elapsed += dt;
          let completed = false; // 处理可能多次触发的情况（当dt > interval时）
