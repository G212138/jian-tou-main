  ...
  return {
    ...
    execute: function () {
      ...
      Command = class Command {
        static create(onComplete, onProgress = null) {
      ...
      Loader = class Loader {
        ...
        load(params) {
          ...
          if (this.assetMap.has(key)) {
            const asset = this.assetMap.get(key); // 有缓存的情况下不触发onProgress回调
          ...
          (_crd && Core === void 0 ? (_reportPossibleCrUseOfCore({
            error: Error()
          }), Core) : Core).inst.manager.loader.load({ ...params,
        ...
        loadDir(params) {
          ...
          (_crd && Core === void 0 ? (_reportPossibleCrUseOfCore({
            error: Error()
          }), Core) : Core).inst.manager.loader.loadDir({ ...params,
            onProgress: (finish, total, item) => {
              if (!this.loadingMap.has(key)) return;
              command.onProgress.forEach(cb => cb(finish, total, item));
            },
        ...
        loadRemote({
          ...
          if (this.assetMap.has(url)) {
            const asset = this.assetMap.get(url); // 有缓存的情况下不触发onProgress回调
