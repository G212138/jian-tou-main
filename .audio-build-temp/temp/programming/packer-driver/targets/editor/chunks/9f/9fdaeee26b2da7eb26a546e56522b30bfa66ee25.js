  ...
  return {
    ...
    execute: function () {
      ...
      _export("Drag", Drag = (_dec = ccclass('Drag'), _dec2 = property({
        ...
        checkPosition() {
          //获取屏幕限制
          let screenSize = this._getScreenLimit(); //获取自己的坐标


          let nodeWorldPos = this.node.worldPosition; //判断是否超出屏幕
        ...
        _onTouchStart(event) {
          this.moveDistance = 0;
          const touches = event.getTouches(); // 如果已经有两个触摸点，则不响应
        ...
        _onTouchMove(event) {
          ...
          if (this._touches.length >= 2 && this._isScaling && this.enableScale) {
            ...
            const touch2 = this._touches[1].getUILocation(); // 计算当前双指距离


            const currentDistance = this._getTouchDistance(touch1, touch2); // 计算距离变化比例


            const distanceRatio = currentDistance / this._lastDistance; // 如果距离变化太小，忽略（可能是手指抖动）
            ...
            let newScale = currentScale * distanceRatio; // 限制在最小最大范围内
            ...
            const currentCenter = new Vec2((touch1.x + touch2.x) / 2, (touch1.y + touch2.y) / 2); // 获取节点当前的世界坐标

            const nodeWorldPos = this.node.worldPosition; // 计算中心点相对于节点中心的偏移

            const offsetX = currentCenter.x - nodeWorldPos.x;
            const offsetY = currentCenter.y - nodeWorldPos.y; // 应用缩放
            ...
            let newWorldY = currentCenter.y - offsetY * scaleRatio; // 应用屏幕限制
          ...
          if (this._touches.length === 1 && this._isDragging && this.enableDrag && !this._isScaling) {
            // 获取触摸点的世界坐标
            const touchPos = this._touches[0].getUILocation(); // 计算新的世界坐标位置（减去偏移）


            let newWorldX = touchPos.x - this._offset.x;
            let newWorldY = touchPos.y - this._offset.y; // 限制在屏幕范围内
        ...
        _onTouchEnd(event) {
          const touches = event.getTouches(); // 移除触摸点
        ...
        _onMouseWheel(event) {
          ...
          const mouseWorldPos = event.getUILocation(); // 获取节点当前的世界坐标

          const nodeWorldPos = this.node.worldPosition; // 计算鼠标相对于节点中心的偏移（世界坐标）

          const offsetX = mouseWorldPos.x - nodeWorldPos.x;
          const offsetY = mouseWorldPos.y - nodeWorldPos.y; // 应用缩放
          ...
          let newWorldY = mouseWorldPos.y - offsetY * scaleRatio; // 应用屏幕限制
        ...
        _getScreenLimit() {
          ...
          const canvasHeight = canvasTransform.height; // 获取节点的尺寸（考虑缩放）

          const selfWidth = selfTransform.width * Math.abs(this.node.scale.x);
          const selfHeight = selfTransform.height * Math.abs(this.node.scale.y); // 计算节点的半尺寸

          const halfWidth = selfWidth / 2;
          const halfHeight = selfHeight / 2; // 计算Canvas的边界（世界坐标）
          ...
          const canvasTop = canvasWorldPos.y + canvasHeight / 2; // 如果节点比屏幕小：限制节点不能离开屏幕
