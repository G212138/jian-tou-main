  ...
  return {
    ...
    execute: function () {
      ...
      _export("PageSpLevel", PageSpLevel = (_dec = ccclass('PageSpLevel'), _dec2 = property(Prefab), _dec3 = property(Node), _dec4 = property(Node), _dec(_class = (_class2 = class PageSpLevel extends (_crd && BaseView === void 0 ? (_reportPossibleCrUseOfBaseView({
        ...
        initLevelStatus() {
          // 从本地存储中读取关卡状态配置
          const savedLevelStatus = (_crd && app === void 0 ? (_reportPossibleCrUseOfapp({
            error: Error()
          ...
          if (savedLevelStatus) {
            // 如果有保存的状态，解析并赋值
            const parsedStatus = JSON.parse(savedLevelStatus); // 确保解析后的数组长度与关卡数量一致
        ...
        onUpdateLevelStatus(levelIndex, status) {
          if (levelIndex >= 0 && levelIndex < this.levelStatus.length) {
            ...
            if (currentLevelItem) {
              const currentLevelItemComp = currentLevelItem.getComponent(_crd && spLevelItem === void 0 ? (_reportPossibleCrUseOfspLevelItem({
                error: Error()
              ...
              if (allPreviousComplete) {
                for (let nextLevelIndex = levelIndex + 1; nextLevelIndex < this.levelStatus.length; nextLevelIndex++) {
                  if (this.levelStatus[nextLevelIndex] === LevelStatus.Unlock) {
                    ...
                    const nextLevelItemComp = nextLevelItem.getComponent(_crd && spLevelItem === void 0 ? (_reportPossibleCrUseOfspLevelItem({
                      error: Error()
                    }), spLevelItem) : spLevelItem);
        ...
        setLevelConfigArrays(arr) {
          ...
          for (let i = 0; i < arr.length; i++) {
            ...
            if (levelConfig) {
              ...
              const levelItemComp = levelItem.getComponent(_crd && spLevelItem === void 0 ? (_reportPossibleCrUseOfspLevelItem({
                error: Error()
              }), spLevelItem) : spLevelItem);
          ...
          for (let i = 1; i < this.levelStatus.length; i++) {
            ...
            if (this.levelStatus[i] === LevelStatus.Unlock) {
              ...
              if (allPreviousComplete) {
                this.levelStatus[i] = LevelStatus.Challenge; // 刷新当前关卡的显示
        ...
        onShow(params) {
          ...
          const loading = (_crd && app === void 0 ? (_reportPossibleCrUseOfapp({
            error: Error()
